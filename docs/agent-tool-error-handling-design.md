...
- `code`：业务或依赖级稳定错误码，例如 `REPORT_NOT_FOUND`。
...
1. 增加 `ToolErrorCategory`、`RecoveryOwner` 和 `RecoveryAction`。
2. 扩展 `ToolExecutionResult`，保持旧字段兼容。
3. 所有业务工具补齐稳定 `error_code`。
4. 增加统一 `ToolErrorClassifier`。
5. `RuntimeState` 增加通用失败记录、禁止动作和不可用工具状态。
6. Prompt 使用结构化 `allowedActions`，不再只依赖错误文案。
...
1. 落实 `timeoutMs`。
2. 重试支持错误分类、指数退避、jitter 和 retry budget。
3. 接入 CircuitBreaker 和 Bulkhead。
4. 增加 OpenTelemetry Span、指标和告警。
5. 分离工具重试次数与计划修订次数。
