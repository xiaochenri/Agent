...
public class StockFinancialToolSemanticValidator implements ToolSemanticValidator {
    ...
    public List<String> validateInput(AgentToolDefinition definition, JsonNode input) {
        ...
        if ("financial_metric_calculator".equals(definition.getName())) {
            ...
            FinancialReportPeriodResolver.ResolvedPeriod resolved =
                    FinancialReportPeriodResolver.resolve(period).orElse(null);
            String peBasis = input.path("pe_basis").asText("");
            String epsBasis = input.path("eps_basis").asText("").trim().toLowerCase();
            boolean explicitTtmEps = "ttm_eps".equals(epsBasis)
                    || "trailing_twelve_months_eps".equals(epsBasis);
            if ("ttm".equals(peBasis) && !explicitTtmEps) {
                errors.add("pe_basis=ttm 必须显式提供eps_basis=ttm_eps，季度披露EPS不能冒充TTM EPS");
            }
            if (resolved != null && !"ANNUAL".equals(resolved.reportType())
                    && "static_at_price_date".equals(peBasis)) {
                errors.add("非年报EPS不能使用static_at_price_date年度静态PE口径");
            }
        }
        return errors;
    }
