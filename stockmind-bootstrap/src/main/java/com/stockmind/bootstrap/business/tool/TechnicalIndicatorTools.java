...
public class TechnicalIndicatorTools extends StockToolSupport {
    ...
    @AgentTool(name = "technical_indicator_snapshot", title = "技术指标快照", namespace = "finance.technical", category = "technical_evidence", tags = {"stock", "indicators", "readonly"}, inputSchema = TECHNICAL_SCHEMA, description = "未指定单项指标时使用：一次返回均线、MACD、RSI、布林带、ATR和量能。只描述价格趋势、动量与波动，不判断内在价值。")
